...
def plot_violin_distances(tyr_csv_files, met_csv_files):
    ...
    split by TRP and TYR/MET residue numbers (excluding PDB codes).
    ...
    for file in tyr_csv_files:
        ...
        # Create X-axis label combining family name, TRP residue, and TYR residue
        df['Residue ID'] = df.apply(lambda x: f"{family_name}-{x['TRP Residue']}-{x['TYR Residue']}", axis=1)
    ...
    for file in met_csv_files:
        ...
        # Create X-axis label combining family name, TRP residue, and MET residue
        df['Residue ID'] = df.apply(lambda x: f"{family_name}-{x['TRP Residue']}-{x['MET Residue']}", axis=1)
    ...
    axs[0].set_xlabel('TRP-TYR Residue Pair by Family')
    ...
    axs[1].set_xlabel('TRP-MET Residue Pair by Family')
    axs[1].set_ylabel('Distance (Å)')
    axs[1].tick_params(axis='x', rotation=45)

    # Adjust layout to make room for labels
    plt.tight_layout()
    plt.savefig("dist_vio-pairs.pdf")
    plt.show()
