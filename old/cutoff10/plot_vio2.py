...
def plot_violin_distances(tyr_csv_files, met_csv_files):
    ...
    for file in tyr_csv_files:
        ...
        # Create a new column combining Dataset and TRP residue number (extract only TRP number)
        df['Residue ID'] = df['PDB ID/Residue Pair'].apply(lambda x: f"{family_name}-{x.split()[1].split('-')[0]}")  # Get TRP residue number only
