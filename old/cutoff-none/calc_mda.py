...
def calculate_distances(pdb_id, cutoff_distance=None):
    ...
    Returns:
        tuple: Lists of tuples with distances to Tyr and Met (e.g., [('W120-Y160', 6.43)]).
    """
    ...
    for trp_atom in trp_n:
        for tyr_atom in tyr_o:
            ...
            if cutoff_distance is None or distance <= cutoff_distance:
                ...
                tyr_results.append((f"{pdb_id} {trp_resid}-{tyr_resid}", distance))
    ...
    for trp_atom in trp_n:
        for met_atom in met_s:
            ...
            if cutoff_distance is None or distance <= cutoff_distance:
                ...
                met_results.append((f"{pdb_id} {trp_resid}-{met_resid}", distance))
...
def process_pdb_list(pdb_list_file, tyr_csv_output_file, met_csv_output_file, cutoff_distance=None):
    ...
    with open(tyr_csv_output_file, mode='w', newline='') as tyr_csvfile, \
        ...
        tyr_csv_writer.writerow(["PDB ID/Residue Pair", "Distance (Å)"])
        met_csv_writer.writerow(["PDB ID/Residue Pair", "Distance (Å)"])
...
for fam in families:
    ...
    cutoff_distance = None  # Optional cutoff distance, set to None for all distances
